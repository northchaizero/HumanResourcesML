...
**Оценка индекса удволетворённости сотрудников (регрессия)**
    ...
    Цель - SMAPE <= 15
Почему SMAPE?:
- Метрика понятна для бизнеса (чем меньше процент ошибки – тем лучше);
- Одинаково хорошо отражает как завышение предсказаний, так и занижение.

**Предсказание ухода сотрудника из компании (бинарная классификация)**

    Цель - ROC-AUC >= 0.91
Почему ROC-AUC?:
- Устойчива к дисбаллансу классов (в нашем случае ~25/75);
- Не зависит от порога разделения классов -> выбирает более универсальную модель на кросс-валидации.
...
Лучшей моделью с метрикой SMAPE = 11.26 является GradientBoostingRegressor, гиперпараметры можно найти в тетрадке (вкладка 'Обучение моделей' в задаче 1).
...
5. Составление портрета уволившегося сотрудника по статистическим данным:
    ...
    который не видит в компании возможностей для карьерного роста.
